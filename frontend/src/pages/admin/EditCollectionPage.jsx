...
export default function EditCollectionPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetchWithAuth(`/api/v1/admin/collections/${id}`)
        if (!res.ok) throw new Error('Collection not found')
        const data = await res.json()
        setCollection(data)
        setForm(collectionToForm(data))
        setProductIds((data.product_ids ?? []).map(String))
        setImages([
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      productIds.forEach((pid) => fd.append('product_ids[]', pid))
      ...
      if (res.ok) {
        ...
        setError(data.errors?.join(', ') || 'Failed to update collection.')
      }
    } catch {
  ...
  async function handleDelete() {
    if (collection.products_count > 0) {
      setError('Cannot delete a collection that has products. Remove all products first.')
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json().catch(() => ({}))
        setError(data.errors?.join(', ') || 'Failed to delete collection.')
      }
    } catch {
    ...
    return (
      <div className="min-h-screen bg-[#FAF9F7] p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <Link to="/admin/collections" className="text-[#E8365D] font-medium hover:underline">
    ...
    <div className="min-h-screen bg-[#FAF9F7]">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <nav className="text-sm text-stone-500 mb-6">
          ...
          <span className="text-stone-800">{collection?.name ?? 'Edit'}</span>
        </nav>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-stone-500 mt-1">Update your collection details and products.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {activeTab === 'seo' && (
              <SeoTab form={{ ...form, slug: form.slug || collection?.slug }} setForm={setForm} slugSource={form.name} />
            )}
          </div>
