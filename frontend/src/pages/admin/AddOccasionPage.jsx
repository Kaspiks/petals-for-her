...
export default function AddOccasionPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      ;(form.product_ids ?? []).forEach((id) => fd.append('product_ids[]', id))
      ...
      if (res.ok) {
        ...
        setError(data.errors?.join(', ') || data.error || 'Failed to create occasion.')
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#FAF9F7]">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <nav className="text-sm text-stone-500 mb-6">
          ...
          <span className="text-stone-800">New Occasion</span>
        </nav>

        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-stone-900">Create New Occasion</h1>
            <p className="text-stone-500 mt-1">Organize products by event or celebration type.</p>
          </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                    form.active ? 'bg-emerald-100 text-emerald-800 border-2 border-emerald-300' : 'bg-stone-50 text-stone-600 border-2 border-transparent'
                  }`}
                >
                  Published
                </button>
                <button
                  ...
                  className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                    !form.active ? 'bg-stone-200 text-stone-700 border-2 border-stone-400' : 'bg-stone-50 text-stone-600 border-2 border-transparent'
                  }`}
                >
                  Draft
                </button>
