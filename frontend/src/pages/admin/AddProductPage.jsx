...
export default function AddProductPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [collectionsRes, classificationsRes] = await Promise.all([
  ...
  async function handleSubmit(publish = false) {
    ...
    try {
      ...
      if (imageFile) {
        ...
        res = await fetchWithAuth('/api/v1/admin/products', {
          method: 'POST',
  ...
  return (
    <div className="p-8">
      <nav className="text-sm text-stone-500 mb-4">
        ...
        <span className="text-stone-800">Add New</span>
      </nav>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800 mb-2">Add New Scented Product</h1>
          <p className="text-stone-500 mt-1">Configure a new floral arrangement with custom scent profile.</p>
        </div>
        <div className="flex gap-3">
          <button
            ...
            disabled={saving}
            className="px-4 py-2 border border-stone-200 rounded-lg text-stone-700 font-medium hover:bg-stone-50 disabled:opacity-50"
          >
            Save Draft
          </button>
          <button
