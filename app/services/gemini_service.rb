...
class GeminiService
  ...
  def stream_generate(prompt_type:, custom_prompt: nil, context: nil, &block)
    ...
    url = URI("#{API_BASE}/models/#{MODEL}:streamGenerateContent?alt=sse&key=#{@api_key}")
    ...
    begin
      http.request(request) do |response|
        if response.code == "429"
          ...
          Rails.logger.error("Gemini 429 response: #{error_body}")
        ...
        unless response.is_a?(Net::HTTPSuccess)
          ...
          block.call("[Error: Gemini API returned #{response.code}]")
          return
        end
        ...
        response.read_body do |chunk|
          ...
          while (line_end = buffer.index("\n"))
            ...
            begin
              ...
              Rails.logger.warn("Gemini SSE parse error: #{e.message}")
            end
          end
        end
      end
      ...
      if retries <= max_retries
        wait_time = 2**retries
        Rails.logger.warn("Gemini rate limited, retrying in #{wait_time}s (attempt #{retries}/#{max_retries})")
      ...
      block.call("[Rate limited by Gemini API: #{detail}]")
      end
    end
  end
